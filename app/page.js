...
export default function Home() {
  ...
  const whatsappText = `Hello SHOPDEX! I want to order:\n\n${cart
   .map((c) => `• ${c.name} x${c.qty} - ₦${(Number(c.price) * c.qty).toLocaleString()}`)
    ...
    <div style={{ background: "#020617", minHeight: "100vh", color: "white", fontFamily: "system-ui" }}>
      <div style={{ background: "black", color: "white", padding: "12px 15px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "2px solid #1e3a8a", position: "sticky", top: 0, zIndex: 40 }}>
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          <button onClick={() => setShowCart(true)} style={{ background: "#1e293b", color: "white", padding: "8px 14px", borderRadius: "6px", border: "1px solid #1e3a8a", fontWeight: "bold" }}>
            🛒 Cart {cartCount > 0 && <span style={{ background: "#ef4444", borderRadius: "50%", padding: "2px 6px", fontSize: "11px", marginLeft: "5px" }}>{cartCount}</span>}
          </button>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "15px" }}>
        <div style={{ display: "flex", gap: "8px", overflowX: "auto", paddingBottom: "10px", marginBottom: "15px" }}>
          {categories.map((cat) => (
            <button key={cat} onClick={() => setActiveCat(cat)} style={{ whiteSpace: "nowrap", padding: "8px 16px", borderRadius: "20px", border: "1px solid #1e3a8a", background: activeCat === cat? "#1e3a8a" : "#1e293b", color: "white", fontWeight: "bold", fontSize: "13px" }}>
              {cat}
            </button>
          ))}
        </div>
          ...
          {filtered.map((p) => (
            <div key={p.id} style={{ background: "#0f172a", padding: "10px" }}>
              <div onClick={() => setSelected(p)} style={{ cursor: "pointer" }}>
                <img src={p.image_url} style={{ width: "100%", height: "140px", objectFit: "contain", background: "white", borderRadius: "6px" }} alt={p.name} />
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 110, display: "flex", justifyContent: "flex-end" }}>
          <div style={{ background: "#0f172a", width: "100%", maxWidth: "380px", height: "100%", display: "flex", flexDirection: "column", borderLeft: "1px solid #1e3a8a" }}>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "15px" }}>
              {cart.length === 0? (
                <p style={{ textAlign: "center", color: "#64748b", marginTop: "40px" }}>Cart empty.<br />Add products!</p>
              ) : (
                cart.map((c) => (
            ...
            {cart.length > 0 && (
              <div style={{ padding: "15px", background: "#1e293b", borderTop: "1px solid #1e3a8a" }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontWeight: "bold", fontSize: "18px", marginBottom: "12px" }}>
                  <span>Total:</span>
                  <span style={{ color: "#60a5fa" }}>₦{total.toLocaleString()}</span>
                </div>
