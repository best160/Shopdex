  ...
  return (
    <div style={{background:"#020617", minHeight:"100vh", color:"white", fontFamily:"system-ui"}}>
      ...
      <div className="header" style={{background:"black", padding:"10px 12px", display:"flex", justifyContent:"space-between", alignItems:"center", borderBottom:"2px solid #1e3a8a", position:"sticky", top:0, zIndex:40, gap:"10px"}}>
        <div className="header-top">
          ...
          <div style={{display:"flex", gap:"6px"}}>
            <button onClick={()=>setShowCart(true)} style={{background:"#1e293b", color:"white", padding:"7px 12px", borderRadius:"6px", border:"1px solid #1e3a8a", fontWeight:"bold", fontSize:"13px", position:"relative"}}>🛒 {cartCount>0&&<span style={{background:"#ef4444", borderRadius:"50%", padding:"2px 6px", fontSize:"10px", marginLeft:"4px"}}>{cartCount}</span>}</button>
      ...
      <div style={{maxWidth:"1200px", margin:"0 auto", padding:"10px"}}>
        ...
        <div className="banner" style={{background:"linear-gradient(to right, black, #1e3a8a)", padding:"0", borderRadius:"12px", border:"1px solid #1e3a8a", marginBottom:"12px", display:"flex", alignItems:"center", justifyContent:"space-between", overflow:"hidden", minHeight:"120px"}}>
        ...
        <div className="grid">
          {filtered.map(p=>(
            <div key={p.id} style={{background:"#0f172a", padding:"8px"}}>
              ...
              <button onClick={()=>addToCart(p)} style={{width:"100%", background:"#1e3a8a", color:"white", border:"none", padding:"7px", borderRadius:"6px", fontWeight:"bold", marginTop:"6px", fontSize:"11px"}}>ADD TO CART</button>
            </div>
          ))}
        </div>
        {filtered.length===0 && <p style={{textAlign:"center", color:"#64748b", marginTop:"30px"}}>No products found in {activeCat}</p>}
      </div>
      ...
      {selected && (
        <div onClick={()=>setSelected(null)} style={{position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100, padding:"15px"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#1e293b", maxWidth:"460px", width:"100%", borderRadius:"12px", overflow:"hidden", border:"1px solid #1e3a8a"}}>
            <img src={selected.image_url} style={{width:"100%", height:"250px", objectFit:"contain", background:"white"}} alt={selected.name}/>
            <div style={{padding:"16px"}}>
              <p style={{fontSize:"11px", color:"#60a5fa", margin:0}}>{selected.category}</p>
              <h2 style={{margin:"4px 0 8px 0", fontSize:"16px"}}>{selected.name}</h2>
              <p style={{fontSize:"18px", fontWeight:"bold", color:"#60a5fa", margin:"0 0 10px 0"}}>₦{Number(selected.price).toLocaleString()}</p>
      ...
      {showCart && (
        <div style={{position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", zIndex:110, display:"flex", justifyContent:"flex-end"}}>
          <div style={{background:"#0f172a", width:"100%", maxWidth:"380px", height:"100%", display:"flex", flexDirection:"column", borderLeft:"1px solid #1e3a8a"}}>
            <div style={{padding:"16px", background:"black", borderBottom:"1px solid #1e3a8a", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
            ...
            <div style={{flex:1, overflowY:"auto", padding:"12px"}}>
              {cart.length===0? <p style={{textAlign:"center", color:"#64748b", marginTop:"40px"}}>Cart empty</p> :
                cart.map(c=>(
                  <div key={c.id} style={{display:"flex", gap:"10px", background:"#1e293b", padding:"10px", borderRadius:"8px", marginBottom:"8px", border:"1px solid #1e3a8a33"}}>
                    <img src={c.image_url} style={{width:"50px", height:"50px", objectFit:"contain", background:"white", borderRadius:"5px"}} alt={c.name}/>
                    <div style={{flex:1}}>
                      <p style={{fontSize:"11px", margin:"0 0 3px 0", overflow:"hidden", whiteSpace:"nowrap", textOverflow:"ellipsis", maxWidth:"180px"}}>{c.name}</p>
                      <p style={{fontSize:"12px", fontWeight:"bold", color:"#60a5fa", margin:0}}>₦{Number(c.price).toLocaleString()}</p>
                      <div style={{display:"flex", gap:"6px", marginTop:"6px", alignItems:"center"}}>
                        <button onClick={()=>changeQty(c.id,-1)} style={{background:"#020617", color:"white", border:"1px solid #1e3a8a", width:"22px", height:"22px", borderRadius:"4px"}}>-</button>
                        <span style={{fontSize:"12px", fontWeight:"bold"}}>{c.qty}</span>
                        <button onClick={()=>changeQty(c.id,1)} style={{background:"#020617", color:"white", border:"1px solid #1e3a8a", width:"22px", height:"22px", borderRadius:"4px"}}>+</button>
            ...
            {cart.length>0 && (
              <div style={{padding:"14px", background:"#1e293b", borderTop:"1px solid #1e3a8a"}}>
                <div style={{display:"flex", justifyContent:"space-between", fontWeight:"bold", fontSize:"16px", marginBottom:"10px"}}><span>Total:</span><span style={{color:"#60a5fa"}}>₦{total.toLocaleString()}</span></div>
                <a href={`https://wa.me/2349059791761?text=${encodeURIComponent(whatsappText)}`} target="_blank" style={{display:"block", background:"#22c55e", color:"white", textAlign:"center", padding:"13px", borderRadius:"8px", textDecoration:"none", fontWeight:900, fontSize:"14px"}}>CHECKOUT ON WHATSAPP</a>
                <button onClick={()=>{setCart([]); setShowCart(false);}} style={{width:"100%", marginTop:"8px", background:"transparent", color:"#64748b", border:"1px solid #334155", padding:"9px", borderRadius:"8px", fontSize:"11px"}}>Clear Cart</button>
              </div>
            )}
          </div>
        </div>
